...
#   res=f.read()
res = requests.get("https://epg.51zmt.top:8001/multicast/").content
...
def findIcon(m, id):
    for v in m:
        if v["name"] == id:
            return urljoin('https://epg.51zmt.top:8001', v["icon"])
...
def loadIcon():
    res = requests.get("https://epg.51zmt.top:8001").content
    m=[]
    #res=""
